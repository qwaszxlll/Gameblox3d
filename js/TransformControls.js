...
( function () {
	...
	var GizmoMaterial = function ( parameters ) {
		...
		this.activate = function(activated) {
			if (activated) {
				this.opacity = this.oldOpacity;
			}
			else {
	...
	THREE.TransformControls = function ( camera, domElement ) {
		...
		this.setTranslationOffset = function(x, y, z) {

			scope.translationOffset = [x, y, z];
		...
		this.setXBound = function( xMin, xMax ){

			scope.xBound = [xMin, xMax];

		}

		this.setYBound = function( yMin, yMax ){

			scope.yBound = [yMin, yMax];
			
		}

		this.setZBound = function( zMin, zMax ){

			scope.zBound = [zMin, zMax];
		...
		function onPointerMove( event ) {
			...
			point.y = Math.max(scope.yBound[0], Math.min(scope.yBound[1], point.y));

			if (scope.xBound != null){
				point.x = Math.max(scope.xBound[0], Math.min(scope.xBound[1], point.x))
			}
			if (scope.zBound != null){
				point.z = Math.max(scope.zBound[0], Math.min(scope.zBound[1], point.z))
			}
			...
			if ( _mode === "translate" ) {
				...
				if ( scope.translationSnap !== null ) {
					...
					if ( scope.axis.search( "X" ) !== - 1 ) scope.object.position.x = Math.round( scope.object.position.x / scope.translationSnap ) * scope.translationSnap + scope.translationOffset[0];
					if ( scope.axis.search( "Y" ) !== - 1 ) scope.object.position.y = Math.round( scope.object.position.y / scope.translationSnap ) * scope.translationSnap + scope.translationOffset[1];
					if ( scope.axis.search( "Z" ) !== - 1 ) scope.object.position.z = Math.round( scope.object.position.z / scope.translationSnap ) * scope.translationSnap + scope.translationOffset[2];
